...
def expectation_maximization(t, nbclusters=2, nbiter=3, normalize=False,\
    ...
    def pnorm(x, m, s):
        ...
        xm = np.matrix(x-m)
        ############If determinant of s is zero, we can calculate a trace?############
        if np.linalg.det(s)==0.0:
            rootDETinv = np.exp(-1*(np.log(np.diag(s))).sum()/2.0)
        else:
            rootDETinv = 1.0/math.sqrt(np.linalg.det(s))
        ##############################################################################
        return (2.0*math.pi)**(-len(x)/2.0)*rootDETinv\
