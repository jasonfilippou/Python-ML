...
def to_diag2(cv):
    '''Convert full covariance matrix to double-diagonal
    
    Input: Full covariance matrix: n_features X n_features
    Only take diag(0), diag(1) and diag(-1), set other terms to zero
    Output: Diag2 covariance matrix: n_features X n_features
    '''
    cv2 = np.diag(np.diag(cv)) + np.diag(np.diag(cv,1),1) + np.diag(np.diag(cv,-1),-1) #+ np.diag(np.diag(cv,2),2) + np.diag(np.diag(cv,-2),-2) + np.diag(np.diag(cv,3),3) + np.diag(np.diag(cv,-3),-3)+ np.diag(np.diag(cv,4),4) + np.diag(np.diag(cv,-4),-4)
    #print "CV2 = \n",cv2 
    return cv2
...
def _log_multivariate_normal_density_full(X, means, covars, min_covar=1.e-7):
    ...
    for c, (mu, cv) in enumerate(itertools.izip(means, covars)):
        cv_log_det = np.sum(np.log(linalg.det(cv)))
        cv_inv_diag2 = to_diag2(linalg.inv(cv))
        L = np.dot((X - mu), cv_inv_diag2)
        log_prob[:,c] = -0.5 * (n_dim * np.log(2 * np.pi) + cv_log_det
                                + np.sum(np.dot(L,(X - mu).T),1) )
    return log_prob

def _log_multivariate_normal_density_full_diag2(X, means, covars, min_covar=1.e-7):
    """Log probability for full covariance matrices.
    """
    from scipy import linalg
    import itertools
    ...
    for c, (mu, cv) in enumerate(itertools.izip(means, covars)):
        if VERBOSE:
            print "k, mu, sigma: ", c, np.shape(mu), np.shape(cv)
        try:
            ...
            cv_chol = linalg.cholesky(cv + min_covar * np.eye(n_dim),
                                      lower=True)
        if VERBOSE:
            print "cv_chol", np.shape(cv_chol)
        cv_log_det = 2 * np.sum(np.log(np.diagonal(cv_chol)))
        if VERBOSE:
            print "cv_log_det:",cv_log_det
        #cv_sol = solve_triangular(cv_chol, (X - mu).T, lower=True).T
        '''Only this line needs to be changed to take inverse and call to_diag2() for solution'''
        cv_sol = np.dot(to_diag2(np.linalg.inv(cv_chol)), (X - mu).T).T
        if VERBOSE:
            print "cv_sol", np.shape(cv_sol)
        log_prob[:, c] = - .5 * (np.sum(cv_sol ** 2, axis=1) + \
                                     n_dim * np.log(2 * np.pi) + cv_log_det)
        if VERBOSE:
            print "cv_sol**2", np.shape(cv_sol**2)
            print "np.sum(cv_sol ** 2, axis=1)", np.shape(np.sum(cv_sol ** 2, axis=1))
            print "log_prob[:, c]", np.shape(log_prob[:, c])

    if VERBOSE:
        print "Done log_prob", np.shape(log_prob)
    return log_prob

def _log_multivariate_normal_density_full_backup(X, means, covars, min_covar=1.e-7):
    """Log probability for full covariance matrices.
    """
    from scipy import linalg
    import itertools
