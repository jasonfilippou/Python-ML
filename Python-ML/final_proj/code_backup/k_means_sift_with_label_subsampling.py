...
def extractLabelInfo(data):
    ...
    categoryMeans = [np.mean(labelDataHash[cat], axis = 0) for cat in range(1, 102) if cat in labelDataHash ]
    if len(categoryMeans) != 101:
        raise LogicalError, "Method %s: categoryMeans array was not computed properly." %(stack()[0][3])
    
    return categoryMeans, exampleHash
...
def getRandomSample(data, exampleFractionPerClass, classLabels):
    ...
    @param exampleFractionPerClass: a float representing the fraction of examples to include from every class.
    @param classLabels: a list with the class labels to get example fractions from.
    @raise DatasetError if data is None or empty
    @raise LogicalError if exampleFractionPerClass is an invalid fraction.
    ...
    if exampleFractionPerClass < 0 or exampleFractionPerClass > 1:
        raise LogicalError, "Method %s: Number of examples provided is invalid." %(stack()[0][3])
    
    sample = []
    for label in classLabels:
        examplesOfCurrentLabel = [ex for ex in data if ex[-1] == label] # retrieve all examples of the current label
        np.random.permutation(examplesOfCurrentLabel) #randomly permute them
        examplesToRetrieve = int(np.floor(exampleFractionPerClass * len(examplesOfCurrentLabel)))# consider only a fraction of them for the sample
        sample.append(np.array(examplesOfCurrentLabel[:examplesToRetrieve]))
        
    return [np.array(l) for group in sample for l in group]
...
#        fp.close()
        ...
        for iteration in range(10):
            ...
            currentExamples = np.array(getRandomSample(SIFTData, 0.5, range(1, 102)))
            ...
            errorRate, goodClusters = evaluateClustering(codebook, currentExamples, assignments, categoryMeans, exampleHash, 101)
